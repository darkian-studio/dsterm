...
### Fixed
- On Windows, `canonicalize()` returns verbatim (`\\?\`) paths that the OS
  treats literally, breaking `/fs/list`, `/fs/read`, and `/fs/stat` for the
  root (os error 3) and leaving `..` traversal un-normalized. Strip the
  extended-length prefix and short-circuit the root request in `safe_path`.
